                        ...
                        <div class="p-6 bg-white border-b border-gray-200">
                            <form method="POST" action="{{ route('stories.store') }}">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="border border-solid border-gray-300 rounded p-2 w-9/12 @error('title') is-invalid border-red-500 @enderror" value="{{ old('title') }}" name="title">
                                    @error('title')
                                ...
                                <div class="mb-8">
                                    ...
                                    <textarea name="body" class="border border-solid border-gray-300 rounded @error('body') is-invalid border-red-500 @enderror" value="">{{ old('body') }}</textarea>
                                    @error('body')
                                        <div class="alert alert-danger flex items-center font-medium tracking-wide text-red-500 text-sm mt-1 ml-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                        ...
                                        <div class="flex justify-right">
                                            <div class="grow">
                                                <div class="form-check">
                                                    <input class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer @error('status') is-invalid border-red-500 @enderror" type="radio" name="status" value="1" {{ '1' == old('status')? 'checked' : '' }} id="flexRadioDefault2">
                                                    <label class="form-check-label inline-block text-gray-800" for="flexRadioDefault2">
                                                        Activated
                                                    </label>
                                                </div>    
                                                <div class="form-check">
                                                    <input class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer @error('status') is-invalid border-red-500 @enderror" type="radio" name="status" value="0" {{ '0' == old('status')? 'checked' : '' }} id="flexRadioDefault1">
                                ...
                                <div class="flex pt-10">
                                    <button role="submit" class="px-6 py-1.5 bg-green-500 text-white hover:bg-green-400 rounded font-bold">CREATE</button>
                                </div>
                            </form>
                        </div>
                    </div>
